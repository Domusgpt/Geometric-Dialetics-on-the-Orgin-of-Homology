...
\begin{abstract}
We provide the first complete algebraic characterization of the Moxness E8$\to$H4 folding matrix. The coefficient triple $(a,b,c) = (\frac{1}{2}, \frac{\varphi-1}{2}, \frac{\varphi}{2})$, where $\varphi = (1+\sqrt{5})/2$, is geometrically required by icosahedral symmetry---any valid E8$\to$H4 projection must involve $\varphi$ because the 600-cell vertices require it.

For this matrix, we document: (1) Row norms $\|U_L\| = \sqrt{3-\varphi}$ and $\|U_R\| = \sqrt{\varphi+2}$, satisfying $\|U_L\| \cdot \|U_R\| = \sqrt{5}$; (2) The $\varphi$-scaling relationship $U_L = (1/\varphi) U_R$ between blocks; (3) Column norms exhibiting duality with row norms; (4) Matrix rank 7 with one-dimensional null space. These properties follow from geometric constraints of E8$\to$H4 projection and are verified computationally to machine precision. We show that arbitrary $\varphi$-containing coefficients do \emph{not} produce these identities---only the geometrically-required Moxness coefficients yield the $\sqrt{5}$ coupling.
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Empirical verification}: We show that arbitrary $\varphi$-containing coefficients do \emph{not} produce $\sqrt{5}$---only the geometrically-required Moxness coefficients do (see Section 8.4 for data).
    \item \textbf{Structural properties}: The rank, null space, and row-column duality emerge from matrix structure, not from $\varphi$ being in coefficients.
\end{enumerate}
...
\subsection{Column Norm Duality}

A remarkable duality exists between row and column norms.

\begin{proposition}[Column Norm Duality]\label{prop:colnorms}
The column norms of the $\varphi$-coupled folding matrix exhibit the same values as the row norms, but distributed differently:
\begin{enumerate}
    \item Columns 0--3: $\|U^{(j)}\| = \sqrt{3-\varphi}$ for $j \in \{0,1,2,3\}$
    \item Columns 4--7: $\|U^{(j)}\| = \sqrt{\varphi+2}$ for $j \in \{4,5,6,7\}$
\end{enumerate}
\end{proposition}

\begin{proof}
Each column contains entries from both $U_L$ and $U_R$ blocks. For column $j$:
\[
\|U^{(j)}\|^2 = \sum_{i=0}^{7} U_{ij}^2
\]
Direct computation shows columns 0--3 each contain entries whose squared sum equals $3-\varphi$, while columns 4--7 each have squared sum $\varphi+2$.
\end{proof}

\begin{remark}
This row-column duality is structural: the same pair of norms $\{\sqrt{3-\varphi}, \sqrt{\varphi+2}\}$ appears in both the row partition (H4L vs.\ H4R) and the column partition (first half vs.\ second half). This is not a consequence of $\varphi$ being in the coefficients---it emerges from the specific arrangement of signs and positions in the matrix.
\end{remark}
...
\begin{enumerate}
    \item $\mathrm{rank}(U) = 7$
    \item $\det(U) = 0$
\end{enumerate}
Thus $U$ is singular and represents a true projection, not a rotation.
\end{proposition}

\begin{proof}
Numerical computation shows that seven of the eight singular values are non-zero, while one singular value is zero (to machine precision). Equivalently, the reduced row echelon form has exactly 7 pivot columns. The determinant is zero as a consequence of the rank deficiency.
\end{proof}

\subsection{Null Space Structure}

\begin{proposition}[Null Space Relation]\label{prop:nullspace}
The null space of $U$ is one-dimensional, spanned by a vector whose structure involves $\varphi$. The rows of $U$ satisfy the linear dependency:
\[
...
\begin{proof}
Direct verification shows this linear combination yields the zero vector. Since $\mathrm{rank}(U) = 7$, there is exactly one independent linear relation among the 8 rows, and this is it.
\end{proof}

\begin{remark}
The appearance of $\varphi$ coefficients in the null space relation is noteworthy: this is a \emph{discovered} relationship, not a constructed one. Even though $\varphi$ appears in the matrix entries, the specific pattern of which rows participate and with what coefficients ($\varphi, -\varphi, -1, 1$) emerges from the matrix structure.
\end{remark}

\begin{corollary}
The fact that $U$ has rank 7 (not 8) means that the E8$\to$H4 projection genuinely loses one dimension of information. The 240 E8 roots project to a 7-dimensional subspace of the 8-dimensional output space.
\end{corollary}
...
We have provided the first complete algebraic characterization of the Moxness E8$\to$H4 folding matrix. For the geometrically-required coefficient triple $(a,b,c) = (1/2, (\varphi-1)/2, \varphi/2)$, we documented the following properties:
...
\begin{enumerate}
    ...
    \item \textbf{Column norm duality} (Proposition \ref{prop:colnorms}): Columns 0--3 have norm $\sqrt{3-\varphi}$; columns 4--7 have norm $\sqrt{\varphi+2}$

    \item \textbf{Rank structure} (Proposition \ref{prop:rank}): $\mathrm{rank}(U) = 7$, $\det(U) = 0$

    \item \textbf{Null space} (Proposition \ref{prop:nullspace}): One-dimensional, with $\varphi$-coefficients in the linear dependency
\end{enumerate}

These results follow from elementary algebraic identities involving the golden ratio, particularly $(3-\varphi)(\varphi+2) = 5$ and $\varphi - 1 = 1/\varphi$. The $\varphi$-scaling structure indicates that E8$\to$H4 folding does not produce two independent H4 copies, but rather one H4 structure viewed at two $\varphi$-related scales.

Crucially, arbitrary $\varphi$-containing coefficients do \emph{not} produce these identities; only the geometrically-required Moxness coefficients do. This confirms that our results document genuine structural properties of E8$\to$H4 folding, not numerical coincidences.

To our knowledge, these specific algebraic characterizations have not been previously published.
