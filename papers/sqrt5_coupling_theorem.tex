...
Row Norms, Rank, and the $\sqrt{5}$-Coupling Identity}
...
\begin{abstract}
We analyze an E8$\to$H4 folding matrix constructed with golden-ratio-coupled coefficients $(a,b,c) = (\frac{1}{2}, \frac{\varphi-1}{2}, \frac{\varphi}{2})$, where $\varphi = (1+\sqrt{5})/2$. These coefficients are geometrically motivated by icosahedral symmetry---any valid E8$\to$H4 projection must involve $\varphi$ because the 600-cell vertices require it.

For this matrix, we document: (1) Row norms $\|U_L\| = \sqrt{3-\varphi}$ and $\|U_R\| = \sqrt{\varphi+2}$, satisfying $\|U_L\| \cdot \|U_R\| = \sqrt{5}$; (2) The $\varphi$-scaling relationship $U_L = (1/\varphi) U_R$ between blocks; (3) Matrix rank 4, consistent with 8D$\to$4D projection. These properties follow from geometric constraints and are verified computationally to machine precision. We show that arbitrary $\varphi$-containing coefficients do \emph{not} produce these identities---only the geometrically-motivated coefficients yield the $\sqrt{5}$ coupling.
\end{abstract}
...
\subsection{Why $\varphi$ Appears: Addressing Circularity}

Before proceeding, we address a natural objection: ``If $\varphi$ appears in the matrix coefficients, isn't finding $\varphi$ in the results circular?''

The answer is no, for a fundamental reason: \textbf{the geometry forces $\varphi$.} The Moxness coefficients are not arbitrary choices---they arise from projecting E8 onto H4 (icosahedral symmetry). The 600-cell vertices \emph{require} the golden ratio; any correct E8$\to$H4 projection must involve $\varphi$.

Our contribution is not claiming that $\varphi$ appeared mysteriously, but rather:
\begin{enumerate}
    \item \textbf{Documenting specific forms}: The exact expressions $\sqrt{3-\varphi}$ and $\sqrt{\varphi+2}$ for row norms are not obvious from the coefficients.
    \item \textbf{Proving the $\sqrt{5}$ identity}: The fact that $(3-\varphi)(\varphi+2) = 5$ is a genuine algebraic theorem requiring derivation.
    \item \textbf{Empirical verification}: We show that arbitrary $\varphi$-containing coefficients do \emph{not} produce $\sqrt{5}$---only the geometrically-motivated coefficients do (see Section 8.4 for data).
    \item \textbf{Structural properties}: The rank 4 and the $\varphi$-scaling between blocks emerge from matrix structure.
\end{enumerate}

This paper characterizes geometrically-required structure, not numerological coincidence.

\subsection{Organization}

Section 2 reviews the necessary background on E8, H4, and folding projections. Section 3 defines the specific matrix family we study and derives its row norm properties. Section 4 states and proves our main theorem. Section 5 documents rank and null space structure. Section 6 presents the $\varphi$-hierarchy. Section 7 provides computational verification. Section 8 discusses geometric interpretation and open questions.
...
\subsection{Column Norm Structure}

The column norms exhibit a more complex pattern than the row norms.

\begin{proposition}[Column Norms]\label{prop:colnorms}
The column norms of the $\varphi$-coupled folding matrix are:
\begin{enumerate}
    \item Columns 0 and 4: $\|U^{(j)}\| = \sqrt{\varphi+2}$ (the H4R row norm)
    \item Columns 1, 2, 5, and 6: $\|U^{(j)}\| = \sqrt{2.5} = \frac{\sqrt{10}}{2}$
    \item Columns 3 and 7: $\|U^{(j)}\| = \sqrt{3-\varphi}$ (the H4L row norm)
\end{enumerate}
\end{proposition}

\begin{proof}
Direct computation of each column's squared norm. For example, Column 0 contains four entries of $|a| = 1/2$ (from U_L) and four entries of $|c| = \varphi/2$ (from U_R), giving $\|U^{(0)}\|^2 = 4(1/4) + 4(\varphi^2/4) = 1 + \varphi^2 = \varphi + 2$.
\end{proof}

\begin{remark}
Unlike the simple row partition (all U_L rows share one norm, all U_R rows share another), the columns exhibit three distinct norm values. The row norms $\sqrt{3-\varphi}$ and $\sqrt{\varphi+2}$ still appear in columns 3, 7 and 0, 4 respectively, while the remaining columns have the intermediate norm $\sqrt{2.5}$.
\end{remark}

\section{Rank and Null Space Structure}

\subsection{Matrix Rank}

\begin{proposition}\label{prop:rank}
The $\varphi$-coupled folding matrix $U$ has:
\begin{enumerate}
    \item $\mathrm{rank}(U) = 4$
    \item $\det(U) = 0$
\end{enumerate}
Thus $U$ is singular with a 4-dimensional null space.
\end{proposition}

\begin{proof}
Numerical computation via singular value decomposition shows exactly four non-zero singular values (approximately $2.62, 2.24, 2.24, 1.77$), with the remaining four being zero to machine precision. The determinant is zero as a consequence of this rank deficiency.
\end{proof}

\begin{remark}
The rank being exactly 4 (half of 8) is significant: the matrix $U$ projects 8-dimensional space onto a 4-dimensional subspace, consistent with its role as an E8$\to$H4 folding matrix. Each 4-dimensional block (U_L and U_R) contributes to this same 4-dimensional image.
\end{remark}

\subsection{Null Space Structure}

\begin{proposition}[Null Space Relation]\label{prop:nullspace}
The rows of $U$ satisfy the linear dependency:
\[
\varphi \cdot \text{Row}_0 - \varphi \cdot \text{Row}_3 - \text{Row}_4 + \text{Row}_7 = \mathbf{0}
\]
This is one of four independent linear relations among the 8 rows.
\end{proposition}

\begin{proof}
Direct verification shows this linear combination yields the zero vector. Since $\mathrm{rank}(U) = 4$, there are exactly four independent linear relations among the 8 rows, of which this is one.
\end{proof}

\begin{remark}
The appearance of $\varphi$ coefficients in this null space relation is noteworthy: this is a \emph{discovered} relationship, not a constructed one. Even though $\varphi$ appears in the matrix entries, the specific pattern of which rows participate and with what coefficients ($\varphi, -\varphi, -1, 1$) emerges from the matrix structure.
\end{remark}

\begin{corollary}
The fact that $U$ has rank 4 means that the E8$\to$H4 projection maps 8-dimensional input to a 4-dimensional output, losing exactly half the dimensions---precisely what is expected for an 8D$\to$4D folding operation.
\end{corollary}
...
Moxness \cite{moxness2014} introduced folding matrices for E8$\to$H4 projection in the context of visualization. The coefficient triple $(a,b,c) = (1/2, (\varphi-1)/2, \varphi/2)$ produces golden-ratio scaling between the H4L and H4R blocks.

\textbf{Important clarification:} Moxness's later work \cite{moxness2014} describes an $8 \times 8$ \emph{unimodular rotation matrix} with $\det(U) = 1$, orthogonality ($U U^T = I$), and palindromic characteristic polynomial. The matrix we analyze here has $\det(U) = 0$ and rank 4---it is a \emph{projection}, not a rotation. While both matrices use the same golden-ratio coefficient structure, they differ in their overall construction and properties.

Our contribution is the algebraic characterization of the coefficient structure itself: when entries use $(a, a/\varphi, a\varphi)$ with the specific sign pattern of Definition \ref{def:matrix}, the $\sqrt{5}$-coupling and $\varphi$-scaling identities hold regardless of whether the full matrix is a rotation or projection. The identities $\|U_L\| \cdot \|U_R\| = \sqrt{5}$ and $\text{Row}_L = (1/\varphi)\text{Row}_R$ follow directly from the coefficient relationships.
...
\subsection{Empirical Test: Alternative Coefficients}

To demonstrate that the $\sqrt{5}$ coupling is not an automatic consequence of using $\varphi$-related coefficients, we tested alternative coefficient choices:

\begin{center}
\begin{tabular}{lccc}
\toprule
Coefficients $(a, b, c)$ & Norm Product & Coupling & $\sqrt{\text{integer}}$? \\
\midrule
Moxness $(0.5, 0.309, 0.809)$ & $\sqrt{5} \approx 2.236$ & $1$ & \textbf{YES} \\
Rational $(0.5, 0.3, 0.8)$ & $2.200$ & $1$ & NO \\
Different $(0.5, 0.4, 0.7)$ & $2.203$ & $0.6$ & NO \\
Uniform $(0.5, 0.5, 0.5)$ & $2.000$ & $0$ & YES (trivial) \\
\bottomrule
\end{tabular}
\end{center}

Only the geometrically-required Moxness coefficients---where $b = a/\varphi$ and $c = a\varphi$---produce the clean $\sqrt{5}$ identity. This confirms that the result is specific to the geometrically-required structure.
...
We have analyzed an E8$\to$H4 folding matrix constructed with golden-ratio-coupled coefficients $(a,b,c) = (1/2, (\varphi-1)/2, \varphi/2)$. For this matrix, we documented the following properties:

\begin{enumerate}
    \item \textbf{Row norms} (Proposition \ref{prop:norms}): $\|U_L\| = \sqrt{3-\varphi}$ and $\|U_R\| = \sqrt{\varphi+2}$

    \item \textbf{$\sqrt{5}$-Coupling} (Theorem \ref{thm:main}): $\|U_L\| \cdot \|U_R\| = \sqrt{5}$

    \item \textbf{$\varphi$-Scaling} (Corollary \ref{cor:phiscaling}): $U_L = (1/\varphi) U_R$

    \item \textbf{Rank structure} (Proposition \ref{prop:rank}): $\mathrm{rank}(U) = 4$, $\det(U) = 0$, consistent with 8D$\to$4D projection

    \item \textbf{Null space} (Proposition \ref{prop:nullspace}): Four-dimensional, with at least one linear dependency involving $\varphi$-coefficients
\end{enumerate}

These results follow from elementary algebraic identities involving the golden ratio, particularly $(3-\varphi)(\varphi+2) = 5$ and $\varphi - 1 = 1/\varphi$. The $\varphi$-scaling structure indicates that the H4L and H4R blocks are not independent but represent one H4 structure viewed at two $\varphi$-related scales.

Crucially, arbitrary $\varphi$-containing coefficients do \emph{not} produce these identities; only the geometrically-motivated coefficients with $b = a/\varphi$ and $c = a\varphi$ do. This confirms that our results document genuine structural properties of E8$\to$H4 folding, not numerical coincidences.

We note that our projection matrix differs from Moxness's unimodular rotation matrix (which has $\det = 1$), though both share the same coefficient structure. The $\sqrt{5}$-coupling and $\varphi$-scaling identities depend on the coefficient relationships, not on whether the overall matrix is a rotation or projection.
